...
exports.addContent = async (req, res) => {
  try {
    const { title, description, type, genre, language, release_year, duration_mins,
            rating, thumbnail, banner, required_plan, is_featured, is_trending, is_new } = req.body;
    const [result] = await db.query(
      `INSERT INTO content (title, description, type, genre, language, release_year, duration_mins,
       rating, thumbnail, banner, required_plan, is_featured, is_trending, is_new)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [title, description, type, genre||null, language||'English', release_year||null,
       duration_mins||null, rating||0, thumbnail||null, banner||null, required_plan||'basic',
