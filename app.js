...
function makeCard(item) {
  ...
  const thumb     = item.banner || item.thumbnail || item.thumbnail_url || `https://picsum.photos/seed/${item.id}/400/600`;
...
async function openDetail(id) {
  ...
  try {
    ...
    if (bannerEl) {
      bannerEl.src = item.banner || item.thumbnail || `https://picsum.photos/seed/${id}b/1280/720`;
